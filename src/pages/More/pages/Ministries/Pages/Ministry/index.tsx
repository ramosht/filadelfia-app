...
const Ministry: React.FC = () => {
  ...
  useEffect(() => {
    const getMinistryInfo = async () => {
      setLoading(true);
      try {
        const response = await api.get(`ministry/${params.id}`);
        setData(response.data.ministry);
        setLoading(false);
      } catch (err) {
        setLoading(false);
        Popup.show({
  ...
  return (
    <DefaultTemplate description={data ? data.title : ' '} pageName="Redes">
      {data && (
        <S.Wrapper>
          {data && (
            <View style={{ flex: 1 }}>
